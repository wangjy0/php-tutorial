...
class MySslEchoServer
{
    ...
    public $port, $cafile, $capath, $cert, $pkey, $base, $bev, $listener, $ctx;

    public function __construct($port, $cert, $pkey, $cafile = null, $capath = null, $host = "127.0.0.1")
    {
        $this->port = $port;
        $this->cert = $cert;
        $this->pkey = $pkey;
        ...
        $this->listener = new EventListener($this->base, array(
            $this,
            "ssl_accept_cb"
        ), $this->ctx, EventListener::OPT_CLOSE_ON_FREE | EventListener::OPT_REUSEABLE, - 1, "$host:$port");
        if (! $this->listener) {
            exit("Couldn't create listener\n");
        }
        
        $this->listener->setErrorCallback(array(
            $this,
            "accept_error_cb"
        ));
    }
    ...
    function ssl_accept_cb($listener, $fd, $address, $ctx)
    {
        ...
        $this->bev = EventBufferEvent::sslSocket($this->base, $fd, $this->ctx, EventBufferEvent::SSL_ACCEPTING, EventBufferEvent::OPT_CLOSE_ON_FREE);
